...
public class SqlCostInterceptor implements Interceptor {
    public static  String TIME_LIMIT_KEY= "timelimit";
    public static  String DEFAULT_TIME_LIMIT = "100";
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        try {
            ...
            if (sqlCost >= timeLimit) {
            	logger.error("发现慢 SQL：[{}]执行耗时[ {}ms]", sql, sqlCost);

            } else {
            	logger.info("Execute SQL：[{}]执行耗时[ {}ms]", sql, sqlCost);
            }
        }
    }
    ...
    private String beautifySql(String sql) {
		...
		return SQLUtils.format(sql,sqlType);
    }
